...
//setting all the variables for thread_function global which will be required
//by all the threads and whose values do not change for any thread
float scaled_color_all [131][131][3] ;
int nthreads ;
...
//This is the function passed to the thread_create, based on the
//thread number, it will go through the vertical lines-> thread_num + nthreads
void*  thread_function(void * args)
{
  int thread_num = (int) args; //getting the thread number from fn argument
  
  /* for every pixel */
  for( int px=thread_num; px<width; px = px+nthreads )
    {
	  ...
	  for(int i=0; i<3; i++)
	    scaled_color_all[px][py][i] = scaled_color[i];
        }
    }
...
int main( int argc, char **argv )
{
    ...
    */
    ...
    //setting values for all the global variables
	...
	if(thread_val == 1)
	  {
	    fprintf(stderr,"Error: Thread was not created successfully.");
	    exit(1);
	  }
      }
    ...
    for(int i=0; i<nthreads ; i++)
      {
	int retv = pthread_join(threadId[i], NULL) ;
	if(retv == 1)
	  {
	    fprintf(stderr,"Error: Failed to finish the thread.");
	    exit(1);
	  }
      }

    //print everything after all the threads have finished their work
    for (int px=0; px < width; px++)
      {
	for(int py=0; py < height; py++)
	  {
	    printf( "%.0f %.0f %.0f\n",                                                     
		    scaled_color_all[px][py][0], scaled_color_all[px][py][1], scaled_color_all[px][py][2] );
	  }
	printf("\n") ;
      }
